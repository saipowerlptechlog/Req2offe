...
export class TagselectedcandidateComponent implements OnInit {
  ...
  getSelectedCandidatesList() {
    this.apiservice.getTagselectedcandidates().subscribe(res => {
      this.spinner.hide();
  ...
  saveTAGSelectedCandidate(formData){
    ...
    // var resObj = {};
    this.apiservice.savetagSelectedCandidate(formData.value,this.email_form).subscribe(res => {
        console.log(res);
        ...
        formData.resetForm({req_id:this.req_id})
        this.getSelectedCandidatesList();
    })
    
  }

  getCandidateById(id, raisedby,email){
    this.saveReq_id = id;
    this.saveRaisedBy = raisedby;
    console.log(email)
    this.apiservice.get_selectedCadidates_Details_ById(id,email).subscribe(res => {
  ...
  deleterecord(i,email) {
    ...
    if (result.value) {
    this.apiservice.deleteSelectedCandidates(i,email).subscribe(res => {
    console.log(res);
    this.getSelectedCandidatesList();
    ...
    clear(data){
      data.resetForm({req_id:this.req_id});
